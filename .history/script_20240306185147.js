document.addEventListener('DOMContentLoaded', () => {
    ...
    const SPEED = 5000; // Adjusted for different speeds between floors
    ...
    function requestFloor(floor) {
        if (floor === 0 && requestedFloors.length === 0) {
            // Special case for Ground level, only UP button allowed
            if (!requestedFloors.includes(floor)) {
                requestedFloors.push(floor);
            }
        } else if (floor === 1) {
            // Level 1: Only UP button allowed to request stop
            if (!requestedFloors.includes(floor)) {
                requestedFloors.push(floor);
            }
        } else if (floor === 2 && requestedFloors.length === 0) {
            // Special case for Level 2, only DOWN button allowed
            if (!requestedFloors.includes(floor)) {
                requestedFloors.push(floor);
            }
        }
    ...
    function processNextFloor() {
        ...
        // Determine the direction based on the next requested floor
