...
function moveElevator(targetLevel, downButtonPressed = false) {
  ...
  if (downButtonPressed) {
    if (currentLevel === 1 && targetLevel === 1) {
      // If DOWN button pressed on level 1, stop at level 1
      speed = 5000; // 5 seconds to reach level 1
    } else {
      // If DOWN button pressed on level 2, skip level 1 and reach Ground level
      speed = 10000; // 10 seconds to skip level 1
    }
  } else {
    // If UP button pressed, calculate speed based on distance
