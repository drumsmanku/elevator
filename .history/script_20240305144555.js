...
function elevatorToLevel(level, timeout) {
  const elevator = document.getElementById('elevator');
  
  // Calculate the transition time based on the timeout.
  // The timeout dictates how long until we start moving, transitionDuration defines the moving speed.
  const transitionDuration = (level * 5000) / level; // Ensure movement speed is consistent per level
  elevator.style.transition = `bottom ${transitionDuration}ms linear`;

  // Timeout wrapper to delay state change for the transition
  setTimeout(() => {
