document.addEventListener('DOMContentLoaded', () => {
    ...
    function processNextFloor() {
        ...
        // Check if we should stop at the next floor
        if ((currentFloor === 1 && nextFloor === 2 && !requestedFloors.some(request => request.floor === 1 && request.direction === 'up')) ||
            (currentFloor === 1 && nextFloor === 0 && !requestedFloors.some(request => request.floor === 1 && request.direction === 'down'))) {
            // Skip the floor
            console.log(`Skipping floor ${nextFloor}`);
            nextFloor = (nextRequest.direction === 'up') ? 2 : 0;
        }
